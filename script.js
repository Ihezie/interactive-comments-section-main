...
function populateWebpage(currentUserData, commentData){
    ...
    if(commentData.replies.length > 0){
      ...
      const repiesCon = document.createElement('div');
      repiesCon.classList.add('replies-con');
      repiesCon.setAttribute('id', `replies-to-${commentData.id}`)
      ...
      repliesData.forEach(replyData=>{
        ...
        repiesCon.appendChild(replyElement);
      })
      commentsCon.appendChild(repiesCon);
    }
}
function constructElement(currentUserData, elementData, elementType){
  ...
  <div class="score">${elementData.score}</div>
  ...
  <div class="${elementType}-info info">
    <img src="${elementData.user.image.png}" class="avatar" alt="user">
    <span class="username">${elementData.user.username}</span>
    <span class="time-stamp">${elementData.createdAt}</span>
  </div>
  ...
  <p class="content">${elementData.content}</p>`
  ...
  if(currentUserData.username === elementData.user.username){
    element.classList.add('current-user-reply');
    ...
    replaceWith.classList.add('edit-delete-con');
